...
* **System-tray app** (`-tray`, Windows): the daemon runs windowless behind a tray icon (`syncdrive-tray.exe`, built `-H=windowsgui`) whose menu opens the web UI, shows the log file, and shuts down cleanly. The `-log` flag tees logs to a file (a GUI process has no console); an `ignoreErr` stderr wrapper stops `io.MultiWriter` short-circuiting when the invalid GUI stderr handle rejects a write. `fyne.io/systray` is pure-Go on Windows and isolated behind `//go:build windows`, so the `CGO_ENABLED=0` Linux CI build is unaffected.
